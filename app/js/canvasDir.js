...
app.directive("drawing", function($document, Companion, ChatService, $firebaseObject, $firebaseArray){ // ChatService,
  return {
    ...
    link: function(scope, element, attrs){
        ...
        function loadCheck(){
          if(objectLoaded && arrayLoaded && messagesLoaded){
            initAll();
            return
          }
        ...
        function initAll(){
          initOthers();
          while (!initOthersComplete){

          }
          console.log("initotherscomplete");
          initPlayer();
          dataUpdate();
          update();
          render();
      ...
      var initOthers = function(){
        ...
        var userOnline;
          for (i=0; i < arrLength; i++){
            ...
            userOnline = false;
            if(syncArray[i].online && otherPlayers[uid] === undefined && uid !== undefined){userOnline = true}
            ...
            if(userOnline){
              ...
              otherPlayers[uid] = {};
              otherPlayers[uid].online = syncArray[i].online;
        ...
        for (m=0;m<otherPlayersUids.length;m++){
          ...
          // }
          if(listPlayer.online){playerOnline = true};
              ...
              if(currentUid){
                for(j = 0; j < syncArray.length; j++){
                  if (syncArray[j].uid === currentUid){
                    otherPlayers[currentUid].online = syncArray[j].online;
